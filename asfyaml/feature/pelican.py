...
class ASFJekyllFeature(ASFYamlFeature, name="pelican", env="production", priority=4):
    ...
    def run(self):
        ...
        print(f"Triggering pelican build on {CI_HOSTNAME}...")
        if not self.noop("pelican"):
            # Contact buildbot 2
            bbusr, bbpwd = open("/x1/gitbox/auth/bb2.txt").read().strip().split(":", 1)
            s = requests.Session()
            s.get(f"https://{CI_HOSTNAME}/auth/login", auth=(bbusr, bbpwd))
            s.post(f"https://{CI_HOSTNAME}/api/v2/forceschedulers/pelican_websites", json=payload)
        else:
            print(payload)
        print("Done!")
...
class ASFJekyllTestFeature(ASFYamlFeature, name="pelican", env="citest", priority=3):
    ...
    def run(self):
        ...
        print(f"Triggering pelican build on {CI_HOSTNAME_TEST}...")
        if not self.noop("pelican"):
            # Contact buildbot 2
            bbusr, bbpwd = open("/x1/gitbox/auth/bb2.txt").read().strip().split(":", 1)
            s = requests.Session()
            s.get(f"https://{CI_HOSTNAME_TEST}/auth/login", auth=(bbusr, bbpwd))
            s.post(f"https://{CI_HOSTNAME_TEST}/api/v2/forceschedulers/pelican_websites", json=payload)
        else:
            print(payload)
        print("Done!")
